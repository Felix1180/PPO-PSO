...
def update_velocity(particle, gbest, w=1, c1=1, c2=0.5):
...
def pso(dimensi, jumlah_partikel, jumlah_iterasi, initial_particles=None):
    ...
    for i in range(jumlah_partikel):
        ax2.plot(iteration_list, [positions[i][0] for positions in position_list], label=f'Particle {i + 1} (x)', marker='o')
        ax2.plot(iteration_list, [positions[i][1] for positions in position_list], label=f'Particle {i + 1} (y)', marker='x')
